...
function isDuckdbConnectionString(connectionString) {
    const raw = String(connectionString || "").trim().toLowerCase();
    if (raw.startsWith("duckdb://"))
        return true;
    if (raw.endsWith(".duckdb") || raw.endsWith(".ddb"))
        return true;
    if (raw === ":memory:" && raw.includes("duckdb"))
        return true;
    return false;
}
...
function detectConnectionDbType(connectionString, savedType) {
    if (savedType) {
        ...
        if (normalized === "turso")
            return "sqlite";
        if (["supabase", "neon", "timescale", "cockroachdb", "yugabytedb", "redshift"].includes(normalized))
            return "postgres";
