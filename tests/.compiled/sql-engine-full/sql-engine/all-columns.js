...
exports.getSqlEngineAllTablesWithColumns = getSqlEngineAllTablesWithColumns;
const detect_1 = require("./detect");
async function getSqlEngineAllTablesWithColumns(connectionString, schema) {
    const engine = (0, detect_1.getSqlEngineKind)(connectionString);
    if (engine === "sqlite")
        return (await import("../sqlite-client")).getSqliteAllTablesWithColumns(connectionString);
    if (engine === "mysql")
        return (await import("../mysql-client")).getAllTablesWithColumns(connectionString, schema);
    if (engine === "postgres")
        return await getPostgresAllTablesWithColumns(connectionString);
    throw new Error("Unsupported SQL engine.");
}
async function getPostgresAllTablesWithColumns(connectionString) {
